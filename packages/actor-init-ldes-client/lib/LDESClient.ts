...
export class LDESClient extends ActorInit implements ILDESClientArgs {
  ...
  `;
    ...
    private async processMembers(members: string[], quadsArrayOfPage: RDF.Quad[]) {
        ...
        const quadArrayPerMember = (await this.mediatorRdfFilterObject.mediate({
        ...
        for(let [id, memberQuadArray] of quadArrayPerMember.entries() ) {
            let memberQuadStream: RDF.Stream & AsyncIterator<RDF.Quad> = await this.quadArrayToQuadStream(memberQuadArray) as RDF.Stream & AsyncIterator<RDF.Quad>
            let outputString;
            if (this.mimeType != "application/ld+json") {
                const handle: IActorQueryOperationOutputQuads = {
                    type: "quads",
                    quadStream: memberQuadStream
                };
